...
from app.services.gemini_service import extract_terms_with_ai
...
class UploadService:
    ...
    def extract_terms(self, user_id: str, upload_id: str) -> list[CandidateTerm]:
        ...
        # Use Gemini AI for intelligent term extraction
        try:
            ai_terms = extract_terms_with_ai(text, max_terms=50)
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"AI term extraction failed: {e}")

        # Cache AI terms in Firestore to avoid re-processing
        doc_ref.update({
    ...
    def generate_cards(self, user_id: str, payload: GenerateCardsRequest) -> str:
        ...
        # Build a lookup from cached AI terms for rich data
        ai_terms_lookup = {}
        cached_ai_terms = data.get("ai_terms", [])
        for ai_term in cached_ai_terms:
            key = self._normalize_term(ai_term.get("term", ""))
            if key:
                ai_terms_lookup[key] = ai_term
        ...
        for term_str in payload.terms:
            ...
            # Look up rich data from Gemini AI terms
            ai_data = ai_terms_lookup.get(normalized_term, {})

            card_req = CardCreateRequest(
                term=term_str.strip(),
                translation=ai_data.get("translation") or "TBD",
